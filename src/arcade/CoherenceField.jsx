  ...
  useEffect(() => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      particles.push({
        ...
        baseFreq: 0.02 + Math.random() * 0.01,
        energy: 0.5 + Math.random() * 0.5,
        r: 2.5 + Math.random() * 1.5,
        baseR: 2.5 + Math.random() * 1.5,
        alive: true,
        consumed: false,
        wallTimer: 0,
      });
    ...
    const tick = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 50 && !v.fear) {
            const pd = q.phase - p.phase,
              c = 0.3 * (1 - dist / 50);
            p.phase += Math.sin(pd) * c * 0.1;
            q.phase -= Math.sin(pd) * c * 0.1;
            p.freq += (q.freq - p.freq) * c * 0.01;
            q.freq -= (q.freq - p.freq) * c * 0.01;
          }
          if (v.fear) {
            p.freq = p.baseFreq;
            q.freq = q.baseFreq;
          ...
          if (v.protect && dist > 20) {
            p.wallTimer = Math.min(p.wallTimer + 0.01, 1);
            p.vx -= nx * 0.015 * p.wallTimer;
            p.vy -= ny * 0.015 * p.wallTimer;
            q.vx += nx * 0.015 * p.wallTimer;
            q.vy += ny * 0.015 * p.wallTimer;
          } else {
            p.wallTimer *= 0.99;
          }
          if (v.consume && dist < 30) {
            if (p.r > q.r) {
              p.r += 0.004;
              q.r -= 0.007;
              q.energy -= 0.002;
        ...
        if (v.neglect) {
          p.energy -= 0.0006;
          p.freq += (Math.random() - 0.5) * 0.002;
          if (p.energy < 0.05) p.alive = false;
        } else {
          p.energy += (1 - p.energy) * 0.005;
        }
        p.vx *= 0.97;
        p.vy *= 0.97;
        ...
        const g = Math.sin(p.phase) * 0.5 + 0.5,
          hue = 170 + Math.sin(p.phase) * 40;
        ...
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.r * (0.7 + g * 0.4), 0, Math.PI * 2);
        ctx.fill();

        ctx.globalAlpha = 1;
      }
      if (!v.consume && t % 50 === 0) {
        const d = particles.find((p) => !p.alive && p.consumed);
        if (d)
          Object.assign(d, {
            x: Math.random() * width,
            y: Math.random() * height,
