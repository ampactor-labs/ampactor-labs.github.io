...
export default function SynthEngine({ width }) {
  ...
  const initAudio = useCallback(() => {
    ...
    feedback.gain.value = params.delayFeed;
  ...
  const noteOn = useCallback(
    (freq) => {
      const ctx = initAudio();
      if (voicesRef.current[freq]) return;
      ...
      env.gain.linearRampToValueAtTime(
        p.sustain * 0.6,
        ctx.currentTime + p.attack + p.decay,
      );
      osc.connect(env);
  ...
  const noteOff = useCallback((freq) => {
    const voice = voicesRef.current[freq];
    if (!voice) return;
  ...
  useEffect(() => {
    const down = (e) => {
      if (e.repeat) return;
      const freq = NOTE_MAP[e.key.toLowerCase()];
      if (freq) {
        noteOn(freq);
  ...
  useEffect(() => {
    if (filterRef.current) {
      filterRef.current.frequency.value = params.cutoff;
      filterRef.current.Q.value = params.resonance;
    }
    ...
    >
      ...
      >
        <Knob
          ...
          max={8000}
          step={10}
          onChange={(v) => setParams((p) => ({ ...p, cutoff: v }))}
        />
        <Knob
          label="RES"
          ...
          max={25}
          step={0.1}
          onChange={(v) => setParams((p) => ({ ...p, resonance: v }))}
        />
        <Knob
          label="ATK"
          value={params.attack}
        ...
        {WHITES.map((k, i) => {
          ...
          return (
            <div
              ...
              onMouseDown={() => {
                noteOn(NOTE_MAP[k.key]);
                setPressedKeys((p) => new Set([...p, k.key]));
              }}
              onMouseUp={() => {
                noteOff(NOTE_MAP[k.key]);
                setPressedKeys((p) => {
                  const s = new Set(p);
                  s.delete(k.key);
                  return s;
        ...
        {BLACKS.map((k) => {
          ...
          return (
            <div
              key={k.key}
              role="button"
              aria-label={`Sharp note ${k.key.toUpperCase()}`}
              aria-pressed={pressed}
              onMouseDown={(e) => {
                e.stopPropagation();
                noteOn(NOTE_MAP[k.key]);
      ...
      {!active && (
        ...
        >
          press a key or tap a note to initialize audio
        </div>
      )}
    </div>
  );
