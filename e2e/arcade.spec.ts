...
test("the whole cabinet is playable from the keyboard", async ({ page }) => {
  ...
  await expect(page.getByRole("option", { selected: true })).toHaveAccessibleName(/MENTL/);

  await page.keyboard.press("ArrowDown");
  await expect(page.getByRole("option", { selected: true })).toHaveAccessibleName(/SONIDO/);
...
test("the coin slot still unlocks the hidden programs", async ({ page }) => {
  ...
  await expect(page.getByRole("option", { name: /SYS\/RESONANCE/ })).toBeVisible();
});

test("the page does not scroll behind the zoomed cabinet", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: "Enter the arcade", exact: true }).click();
