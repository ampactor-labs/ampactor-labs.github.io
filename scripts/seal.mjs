...
function ask(question, { hidden = false } = {}) {
  return new Promise((done, fail) => {
    ...
    if (hidden && !stdin.isTTY) {
      fail(new Error('Refusing to read a passphrase from a pipe. Run this in a terminal.'))
      return
    }
...
for (const file of files) {
  ...
  console.log(`  + ${titleOf(html, file)}  (${(html.length / 1024).toFixed(1)} KB)`)
}
...
console.log('Commit and push; the Pages workflow deploys it to ampactor.dev/keep/')
